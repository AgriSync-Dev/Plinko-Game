...
import { value, ballXValue, getMultiplierValue } from 'utils/control'
...
export function Game() {
  ...
  const addBall = useCallback(
    (ballValue: number) => {
      ...
      //   pinsConfig.pinGap +
      //   pinsConfig.pinGap / 2
      // const ballX = parseInt(random(minBallX, maxBallX))
      const ballX = ballXValue()
  ...
  function bet(betValue: number, nBalls: number) {
    let counter = 0
  ...
  async function onCollideWithMultiplier(ball: Body, multiplier: Body) {
    ...
    if (parseFloat(multiplier.label.split('-')[1]) === getMultiplierValue())
      console.log(ball.label.split('-')[2], '-', multiplier.label.split('-')[1])
